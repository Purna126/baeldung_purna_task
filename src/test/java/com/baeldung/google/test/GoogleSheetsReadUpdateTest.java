...
public class GoogleSheetsReadUpdateTest {
    ...
    public static void main(String... args) throws GeneralSecurityException, IOException {
        ...
        if (values == null || values.isEmpty()) {
            ...
            System.out.println("category_id, name, slug, post_count, category_description");
           /* List<Object> headerNames=values.get(0);
             if(!headerNames.contains(columnName)) {
                 headerNames.add((values.get(0).size()),columnName);
                 addNewColumn(response,range);
             }*/
    ...
    public static void addNewColumn(ValueRange response,String range) throws IOException {
        ValueRange body = new ValueRange().setRange(range)
                .setValues(response.getValues());
        UpdateValuesResponse result = sheetsService.spreadsheets().values()
                .update(SPREADSHEET_ID,"", body)
